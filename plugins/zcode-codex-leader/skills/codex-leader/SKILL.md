...
Substantive commands are compact by default: stdout prints `RESULT_FILE:<path>`, a <=120-word `SUMMARY:`, then a trailing `Plugin evidence:` line. GPT-Pro `start` prints `TASK_ID`/`TASK_FILE`/`RESULT_FILE` plus evidence, then `poll`/`collect`/`cancel` keep stdout compact. Full worker output lives in the result file. Do not use `--print-full` during normal leader operation.
